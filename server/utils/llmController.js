...
const queryPronpt =
  "Imagine you are a seasoned video producer with over 20 years of experience creating content for diverse audiences. Your current task involves selecting background videos from an online platform called 'Pexels' to match short text scripts for video scenes, each lasting 5-8 seconds. Your objective is to identify the essences of each scene from the following text and formulate concise search queries (2-3 words each) that are broad yet relevant to return useful video results Understand the idea of the video and come up with videos that make sense to be one after the other Make sure that the queries are different and diverse from the other. Additionally, provide a backup query for each primary query, consisting of a single, more general word. For instance, for an example scene: 'Struggling to fall asleep? Let's start with a bedtime routine.', an appropriate query response for that scene might be ['bedtime routine', 'sleep'], For the example scene  'Make use of windows, skylights, and light-colored decor to enhance the flow of natural light throughout your home.', [natural light', 'light'] fits well. For each scene in the text provide the two query worlds as describe, separate each [] with “;”(for example for text with 5 scenes your response structure should look like: [];[];[];[];[])";
